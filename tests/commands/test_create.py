import click
from click.testing import CliRunner
from vm_lifecycle.commands.create import create_vm_instance


def test_create_exits_if_config_invalid(mocker):
    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context", return_value=(None, None, None)
    )

    runner = CliRunner()
    result = runner.invoke(create_vm_instance)

    assert result.exit_code == 1
    assert "❗ Error with active profile" in result.output or result.output == ""


def test_create_exits_if_instance_exists(mocker):
    mock_instance = {"name": "test-instance"}
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {"instance_name": "test-instance"}

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = [mock_instance]

    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "europe-west1-b"),
    )

    runner = CliRunner()
    result = runner.invoke(create_vm_instance)

    assert result.exit_code == 1
    assert "already exists" in result.output


def test_create_aborts_on_image_confirm_decline(mocker):
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {
        "instance_name": "test-instance",
        "image_base_name": "test-image",
    }

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = []
    mock_compute.list_images.return_value = [{"name": "img1"}, {"name": "img2"}]

    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "zone"),
    )
    mocker.patch("click.confirm", return_value=False)

    runner = CliRunner()
    result = runner.invoke(create_vm_instance)

    assert result.exit_code == 1
    assert "Aborted" in result.output


def test_create_successful_instance_creation(mocker):
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {
        "instance_name": "test-instance",
        "image_base_name": "test-image",
        "machine_type": "e2-standard-2",
        "disk_size": 100,
        "instance_user": "ubuntu",
    }
    mock_config.update_active_zone_region.return_value = True

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = []
    mock_compute.list_images.return_value = []
    mock_compute.create_instance.return_value = {"name": "operation-123"}

    def mock_poll_with_spinner(*args, **kwargs):
        click.echo(kwargs.get("done_text", "✅ Operation Complete!"))
        return {"success": True}

    mocker.patch(
        "vm_lifecycle.commands.create.poll_with_spinner",
        side_effect=mock_poll_with_spinner,
    )

    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "zone"),
    )
    echo_mock = mocker.patch("vm_lifecycle.commands.create.click.echo")

    runner = CliRunner()
    result = runner.invoke(create_vm_instance)

    assert result.exit_code == 0
    echo_mock.assert_any_call("✅ Instance: 'test-instance' created in zone: 'zone'")


def test_create_prompts_confirmation_if_images_exist(mocker):
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {
        "instance_name": "test-instance",
        "image_base_name": "test-image",
        "machine_type": "e2-standard-2",
        "disk_size": 100,
        "instance_user": "ubuntu",
    }

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = []
    mock_compute.list_images.return_value = [{"name": "img1"}]

    confirm_mock = mocker.patch("click.confirm", return_value=True)
    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "zone"),
    )
    mocker.patch(
        "vm_lifecycle.commands.create.poll_with_spinner", return_value={"success": True}
    )
    mocker.patch("vm_lifecycle.commands.create.click.echo")

    runner = CliRunner()
    result = runner.invoke(create_vm_instance)

    assert result.exit_code == 0
    confirm_mock.assert_called_once()


def test_create_passes_correct_args_to_poll_with_spinner(mocker):
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {
        "instance_name": "test-instance",
        "image_base_name": "test-image",
        "machine_type": "e2-standard-2",
        "disk_size": 100,
        "instance_user": "ubuntu",
    }

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = []
    mock_compute.list_images.return_value = []
    mock_compute.create_instance.return_value = {"name": "op-456"}

    poll_mock = mocker.patch(
        "vm_lifecycle.commands.create.poll_with_spinner", return_value={"success": True}
    )
    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "europe-west1-b"),
    )
    mocker.patch("vm_lifecycle.commands.create.click.echo")

    runner = CliRunner()
    runner.invoke(create_vm_instance)

    poll_mock.assert_called_once_with(
        compute_manager=mock_compute,
        op_name="op-456",
        text="Creating instance: 'test-instance' in zone: 'europe-west1-b'",
        done_text="✅ Instance: 'test-instance' created in zone: 'europe-west1-b'",
        scope="zone",
        zone="europe-west1-b",
    )


def test_create_uses_custom_image_if_provided(mocker):
    mock_config = mocker.MagicMock()
    mock_config.active_profile = {
        "instance_name": "custom-vm",
        "image_base_name": "custom-image",
        "machine_type": "n1-standard-1",
        "disk_size": 50,
        "instance_user": "deployer",
    }

    mock_compute = mocker.MagicMock()
    mock_compute.list_instances.return_value = []
    mock_compute.list_images.return_value = []
    mock_compute.create_instance.return_value = {"name": "create-op"}

    mocker.patch(
        "vm_lifecycle.commands.create.init_gcp_context",
        return_value=(mock_config, mock_compute, "zone"),
    )
    mocker.patch(
        "vm_lifecycle.commands.create.poll_with_spinner", return_value={"success": True}
    )
    mocker.patch("vm_lifecycle.commands.create.click.echo")

    runner = CliRunner()
    runner.invoke(create_vm_instance, ["--image", "my-custom-image"])

    # Ensure correct values passed
    mock_compute.create_instance.assert_called_once_with(
        instance_name="custom-vm",
        machine_type="n1-standard-1",
        disk_size=50,
        instance_user="deployer",
        zone="zone",
        custom_image_name=None,
        image_project=None,
        image_family=None,
        startup_script_type=None,
    )
